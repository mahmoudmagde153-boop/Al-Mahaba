$idx = Get-Content "C:\Users\Mahmoud.Magdy\OneDrive - Nations of sky\Desktop\AlMahaba_System\index.html" -Raw

$headerControlsTarget = '<select class="input-field input-sm" id="attendanceMonthSelect"></select>'
$headerControlsNew = '<select class="input-field input-sm" id="attendanceMonthSelect"></select>`n                          <button class="btn btn-warning btn-sm" onclick="window.Attendance.showAdvanceModal()" style="margin-right: 10px;">`n                              <i class="fas fa-money-bill-wave"></i> تسجيل سلفة`n                          </button>'
$idx = $idx.Replace($headerControlsTarget, $headerControlsNew)

$summaryHeaderTarget = '<th>رسمية</th>'
$summaryHeaderNew = '<th>رسمية</th>`n                                      <th>السلف (ج)</th>'
$idx = $idx.Replace($summaryHeaderTarget, $summaryHeaderNew)

$advanceModalHtml = @"
    <!-- نافذة السلف -->
    <div class="modal-overlay" id="advanceModalOverlay">
        <div class="modal attendance-modal" style="width: 400px; padding: 20px;">
            <div class="modal-header">
                <h3>تسجيل سلفة نقدية</h3>
                <button class="modal-close" onclick="window.Attendance.hideAdvanceModal()">×</button>
            </div>
...
"@
$idx = $idx.Replace('<div class="modal-overlay" id="attendanceModalOverlay">', $advanceModalHtml + "`n    <div class=`"modal-overlay`" id=`"attendanceModalOverlay`">")

# Bump cache to v=10
$idx = $idx.Replace('v=9', 'v=10')

Set-Content "C:\Users\Mahmoud.Magdy\OneDrive - Nations of sky\Desktop\AlMahaba_System\index.html" $idx
...
window.Attendance = (function () {
    ...
    function init() {
        ...
        if (select) {
            currentMonth = select.value || window.App.getCurrentMonth();
        } else {
            currentMonth = window.App.getCurrentMonth();
        }
        render();
        setupEvents();
    }
    ...
    function setupEvents() {
        ...
        if (body) {
            body.addEventListener('click', (e) => {
                ...
                if (!cell) return;
                const { emp, day, month } = cell.dataset;
    ...
    function render() {
        ...
        const daysInMonth = window.App.getDaysInMonth(year, monthNum);
...
Set-Content "C:\Users\Mahmoud.Magdy\OneDrive - Nations of sky\Desktop\AlMahaba_System\js\attendance.js" $attJs


$appJs = Get-Content "C:\Users\Mahmoud.Magdy\OneDrive - Nations of sky\Desktop\AlMahaba_System\js\app.js" -Raw
$appJs = $appJs -replace '(?s)// ربط تغيير الشهر.*?document\.getElementById\(''attendanceTypeGrid''\)\.addEventListener\(''click'', \(e\) => \{.*?\}\);\s*', ''
$appJs = $appJs -replace '(?s)document\.getElementById\(''attendanceMonthSelect''\)\?.addEventListener\(''change'', renderAttendance\);\s*', ''
$appJs = $appJs -replace '(?s)document\.getElementById\(''attendanceBody''\)\.addEventListener\(''click'', \(e\) => \{.*?\}\);\s*', ''
$appJs = $appJs -replace '(?s)document\.getElementById\(''attendanceModalClose''\)\.addEventListener\(''click'', closeAttendanceModal\);\s*', ''
$appJs = $appJs -replace '(?s)document\.getElementById\(''attendanceModalOverlay''\)\.addEventListener\(''click'', \(e\) => \{.*?\}\);\s*', ''

Set-Content "C:\Users\Mahmoud.Magdy\OneDrive - Nations of sky\Desktop\AlMahaba_System\js\app.js" $appJs
